...
알려주기 위한 진단 어휘 추가. 계기는 prod 실계좌의 장외(OTC) 보유 종목이다.

### Added
- `OVERSEAS_STOCK_ORDER_MARKET_CODES` — 해외주식 주문 TR 이 받는 시장 집합(NYSE/AMEX=81,
  NASDAQ=82). `COSAT00301` 의 `OrdMktCode` 와 `g3101` 의 `exchcd` 가 둘 다
  `Literal["81","82"]` 라 **세 번째 값은 없다**. 추측으로 코드를 채우지 않기 위한 단일 출처.
- `unsupported_market_reject()` — 지원 밖 시장 종목에 대한 구조화 거부 진단.
  `retry=do_not_retry`(성공할 수 없는 요청), `rsp_cd=PG_UNSUPPORTED_MARKET` 센티널
  (브로커가 준 코드가 아님을 소비자가 구분할 수 있게 `PG_` 접두).
- `LS_OVERSEAS_DESK_PHONE` (`02-3779-8888`) — OTC 보유 종목은 이 번호로 전화해야 매도된다.
  LS증권 확인: 미국 정규장 시간에 현지에서 주식을 사 갈 브로커를 찾아 매도해 준다.
- `looks_like_otc_ticker()` — LS 경험칙(티커 5자 + 끝이 `F`). ⚠️ LS 도 "항상 맞는 룰은 아니다"
  라고 단서를 달았으므로 **주문 차단에 쓰지 않는다**. 안내 문구를 구체화하는 힌트 전용이고,
  차단 판단은 거래소 코드로만 한다.

### Fixed
- `03053`("해당 종목번호가 없습니다") 안내가 **"종목 철자를 확인하세요" 뿐**이라 오해를 키웠다.
  계좌가 **보유 중인** 종목에서 이 코드가 나면 철자 문제가 아니라 장외 종목이라 거래 자체가
  불가한 경우다. 두 경우를 나눠 설명하고 LS 데스크 연락처를 함께 안내한다.
  (실측: 보유 종목 ZOMDF 를 허용값 `82`·`81` 로 각각 보내 둘 다 03053 거부)
...
> 문장(cause/tip)에서 추론해야 했고, 소비자마다 다르게 추론했다.
> 동반 릴리즈: `finance` **1.9.1** · `programgarden` **1.33.0**.

### Added
- **`RetryAdvice` 4값** — `do_not_retry` / `retry_after_fix` / `retry_later` / `unknown`.
- **`OrderRejectInfo.retry`** (기본 `unknown`) + 실측 등재 7개 코드에 판단 부여:
  `do_not_retry` = 02259·03053·03759·01524·03056(같은 요청 = 같은 답),
  `retry_after_fix` = 02201(예수금)·03181(하한가 미달)(사용자가 바꾸면 통한다).
- 접수됐는데 주문번호가 없는 상태(`diagnose_missing_order_no`)도 `do_not_retry` —
  재전송하면 **살아 있는 주문이 중복 발주**된다. 안전한 행동은 재조회뿐이다.

  🔴 **미등재 코드는 `unknown` 이다.** 여기에 "재시도 가능" 을 기본값으로 두면 접수 여부를
  모르는 주문을 다시 보내 중복 발주가 난다. 대신 `raw_msg`(증권사 원문)를 항상 함께 싣는다.

### Fixed
- **`programgarden_core.__version__` 이 거짓 버전을 말하던 문제** — 손으로 적힌 `1.12.4` 가
  pyproject 의 `1.24.0` 과 어긋난 채였다. `importlib.metadata` 파생으로 바꿔 다시 어긋날 수
  없게 했다. (1.32.0 릴리스 때 "core 릴리스가 필요해 별건" 으로 미뤄 둔 항목 — 이번이 그 릴리스다.)

## [1.24.0] - 2026-08-19
> 실주문 경로 3건 — 1.23.0 배포 이후 소스가 바뀌었는데 버전이 그대로여서 PyPI 배포판에는
> 한 번도 반영된 적이 없었다(로컬 path 의존이 이를 가렸다). `programgarden` 1.31.2 가
> `diagnose_missing_order_no` 를 import 하므로 이 릴리즈가 선행 조건이다.

### Added
- **`diagnose_missing_order_no`** + 거부코드 표 `OVERSEAS_STOCK_REJECT_CODES` /
  `KOREA_STOCK_REJECT_CODES` — 실계좌 실측 코드에 원인·해결책을 붙여 등재한다.
  (해외 `02201` 예수금 부족 / `02259` 원주문 없음 / `03053` 미인식 종목 / `03759`,
  국내 `01524` 미인식 종목 / `03056` 원주문 없음 / `03181` 하한가 미만 등)
  종전엔 거부 원인을 **추측 문구**로 만들었다. LS 업무거부는 `error_msg` 가 빈 문자열이고
  `rsp_cd` 에만 원인이 있어 `if error_msg:` 로는 한 건도 못 잡는다.
- 주문 성공 코드 실측 등재 — 신규매수 `00040` · 신규매도 `00039` · 취소 `00156`.

### Fixed
- **주문 TR 코드 오선언 3건** — 해외주식 취소가 존재하지 않는 `COSAT00303` 을 가리켰다.
  해외주식엔 전용 취소 TR 이 없어 신규주문과 같은 `COSAT00301` 에 취소 유형코드
  (`OrdPtnCode="08"`)를 실어야 한다. 해외선물 정정 `CIDBT00200`→`CIDBT00900`,
  취소 `CIDBT00300`→`CIDBT01000` 으로 교정(라이브 실측).
- **국내 취소 수량 미지정 시 1주만 취소되던 결함** — 국내(CSPAT00801)는 해외와 반대로
  **부분취소**라 `OrdQty` 가 취소할 수량이다. 미지정 시 잔량 전체가 아니라 1주만 취소되어
  "취소 완료"를 받고도 주문이 살아 있었다.
- **빈 취소주문번호를 실패로 판정** — 취소 응답의 주문번호가 0/빈값이면 실패로 처리한다.
  요청측 가드(원주문번호 필수)는 1.31.1 에 이미 있었고, 이번에 막은 건 **응답측**이다 —
  살아 있는 주문을 "취소됨"으로 보고하던 거짓 성공을 차단한다.
...
- **Deep-validation error codes** (3 additive `ErrorCode` enum values) — surfaced by
  `programgarden`'s virtual full-execution deep validation (`ProgramGarden.validate_deep`):
  - `DEEP_VALIDATION_NODE_ERROR` — a node raised/failed during the offline validation pass.
  - `DEEP_VALIDATION_FLOW_BROKEN` — the workflow did not run end-to-end to completion.
  - `DEEP_VALIDATION_BINDING_UNRESOLVED` — an unresolved `{{ }}` template binding was detected.

### Changed
- **Expression evaluator accepts JSON-style literals** — `SafeEvaluator` now resolves the
  bare names `null` / `true` / `false` (aliases of `None` / `True` / `False`) in addition to
  the Python spellings. Workflow definitions are authored as JSON (and frequently AI-generated),
  so an otherwise valid `{{ x != null }}` expression no longer fails with
  "undefined variable: null". Python `None`/`True`/`False` continue to work unchanged.
