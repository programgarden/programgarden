...
class TestSchemaRegistration:
    ...
    def test_register_schema_success(self, sample_schema):
        ...
        assert registry.get_schema("Dynamic_MyRSI") is not None
        assert registry.get_schema("Dynamic_MyRSI").category == "condition"

    def test_register_schema_without_prefix_fails(self):
        """Dynamic_ prefix 없이 등록하면 실패"""
        registry = DynamicNodeRegistry()
    ...
    def test_register_schemas_batch(self):
        ...
        registry = DynamicNodeRegistry()
        schemas = [
            DynamicNodeSchema(node_type="Dynamic_A", category="data"),
            DynamicNodeSchema(node_type="Dynamic_B", category="condition"),
        ]
        registry.register_schemas(schemas)

        assert len(registry.list_schema_types()) == 2
        assert "Dynamic_A" in registry.list_schema_types()
        assert "Dynamic_B" in registry.list_schema_types()
    ...
    def test_list_schemas(self, sample_schema):
        ...
        assert schemas[0].node_type == "Dynamic_MyRSI"

    def test_get_schema_not_found(self):
        """미등록 스키마 조회 시 None 반환"""
        registry = DynamicNodeRegistry()
        assert registry.get_schema("Dynamic_NotExists") is None

    def test_is_schema_registered(self, sample_schema):
        """스키마 등록 여부 확인"""
        registry = DynamicNodeRegistry()

        assert not registry.is_schema_registered("Dynamic_MyRSI")
        registry.register_schema(sample_schema)
        assert registry.is_schema_registered("Dynamic_MyRSI")
...
class TestClassInjection:
    ...
    def test_inject_node_class_success(self, sample_schema):
        ...
        registry.inject_node_class("Dynamic_MyRSI", ValidCustomNode)

        assert registry.get_node_class("Dynamic_MyRSI") is not None
        assert registry.is_class_injected("Dynamic_MyRSI")
    ...
    def test_inject_non_basenode_fails(self, sample_schema):
        ...
        with pytest.raises(TypeError, match="BaseNode를 상속"):
            registry.inject_node_class("Dynamic_MyRSI", NotABaseNode)
    ...
    def test_inject_without_execute_fails(self, sample_schema):
        ...
        with pytest.raises(TypeError, match="execute\\(\\) 메서드"):
            registry.inject_node_class("Dynamic_MyRSI", MissingExecuteNode)
    ...
    def test_inject_with_wrong_outputs_fails(self, sample_schema):
        ...
        with pytest.raises(ValueError, match="output 포트가 클래스에 없음"):
            registry.inject_node_class("Dynamic_MyRSI", WrongOutputsNode)
    ...
    def test_inject_node_classes_batch(self):
        ...
        registry.inject_node_classes({
            "Dynamic_A": ValidCustomNode,
            "Dynamic_B": ValidCustomNode,
        })

        assert registry.is_class_injected("Dynamic_A")
        assert registry.is_class_injected("Dynamic_B")
    ...
    def test_get_node_class_not_injected(self, sample_schema):
        ...
        assert registry.get_node_class("Dynamic_MyRSI") is None
        assert not registry.is_class_injected("Dynamic_MyRSI")
...
class TestUtilities:
    ...
    def test_unregister(self, sample_schema):
        ...
        result = registry.unregister("Dynamic_MyRSI")

        assert result is True
        assert registry.get_schema("Dynamic_MyRSI") is None
        assert registry.get_node_class("Dynamic_MyRSI") is None

    def test_unregister_not_found(self):
        """미등록 노드 제거 시 False 반환"""
        registry = DynamicNodeRegistry()
        assert registry.unregister("Dynamic_NotExists") is False
...
class TestHelperFunctions:
    ...
    def test_is_dynamic_node_type(self):
        """동적 노드 타입 판별"""
        assert is_dynamic_node_type("Dynamic_MyRSI") is True
        assert is_dynamic_node_type("Dynamic_MACD") is True
    ...
    def test_dynamic_node_prefix(self):
        """Prefix 상수 확인"""
        assert DYNAMIC_NODE_PREFIX == "Dynamic_"
...
class TestSingleton:
    ...
    def test_singleton_instance(self, sample_schema):
        ...
        # 데이터 공유
        assert registry2.get_schema("Dynamic_MyRSI") is not None
    ...
    def test_reset_instance(self, sample_schema):
        ...
        registry2 = DynamicNodeRegistry()
        assert registry2.get_schema("Dynamic_MyRSI") is None
