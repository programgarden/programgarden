...
### Fixed
- **체결 조회 메서드를 엉뚱한 클래스에서 부르던 문제** — 재조정기가 매 주기
  `AttributeError: 'WorkflowJob' object has no attribute '_query_overseas_stock_fills_by_date'`
  로 실패했다. 그 메서드는 `NewOrderNodeExecutor` 소유다. 1.37.5 에서 고친
  `_active_trackers`(`BrokerNodeExecutor` 소유)와 **정확히 같은 실수**다.
  `executor.py` 는 한 모듈에 클래스가 수십 개라 `self.x` 가 어느 클래스의 것인지 눈으로
  안 보인다.

### Added (방어)
- **소유자 검증 테스트** — 재조정 메서드가 참조하는 모든 `self.<이름>` 이 `WorkflowJob` 에
  실재하는지 AST 로 확인한다. 개별 이름을 나열하는 게 아니라 **규칙**을 검증하므로 새
  메서드를 추가해도 자동으로 보호된다. 변이 검증으로 두 결함을 모두 잡는 것을 확인했다.
  (주석·docstring 에 남긴 "왜 틀렸는지" 기록은 AST 검사라 오탐하지 않는다.)

### Known issue (여전히 남음)
- 재조정 루프는 이벤트 루프 진입 시 시작한다. 거래시간 게이트가 닫혀 있으면 그 전에 흐름이
  멈춰 재조정이 아예 돌지 않는다. 장 끝난 뒤 체결을 정리하는 것이 본래 목적이므로 거래시간과
  무관하게 돌아야 한다.

deps: core 1.28.1 / finance 1.9.7 / community 1.15.3 (변경 없음)

## [1.37.5] - 2026-09-14

1.37.4 의 체결 재조정기가 **prod 에서 매 주기 죽던 것**의 수정. 릴리스 당일 실관측으로 잡았다.

### Fixed
- **재조정기가 계좌 추적기를 못 찾아 매 주기 실패하던 문제** —
  `AttributeError: 'WorkflowJob' object has no attribute '_active_trackers'`.
  추적기 레지스트리는 `BrokerNodeExecutor` 의 **클래스 속성**(프로세스 전역)인데
  `WorkflowJob` 의 것으로 읽고 있었다. 재조정이 한 건도 돌지 못했다.
  🔴 단위 테스트가 그 속성을 **직접 만들어 넣는** 픽스처를 써서 가려졌다 — 테스트가 만든
  세계에서만 통과하는 코드였다. 픽스처를 실제 레지스트리를 쓰도록 바꾸고, 결함을 되돌리면
  테스트가 깨지는 것까지 확인했다.
- **다른 잡의 계좌 추적기를 집을 수 있던 문제** — 레지스트리 키는 `job_id_node_id` 이고
  레지스트리는 프로세스 전역이다. 한 프로세스에 여러 잡이 뜨면 **남의 계좌 잔고로 이 잡의
  유령 포지션을 판정**하게 된다. job_id 로 거른다.

### Known issue (다음 버전 대상)
- 재조정 루프는 **이벤트 루프 진입 시** 시작한다. 거래시간 게이트(TradingHoursFilterNode)가
  닫혀 있으면 그 전에 흐름이 멈춰 재조정이 아예 돌지 않는다. **장 끝난 뒤 체결을 정리하는
  것이 본래 목적**이므로 거래시간과 무관하게 돌아야 한다.

deps: core 1.28.1 / finance 1.9.7 / community 1.15.3 (변경 없음)

## [1.37.4] - 2026-09-14

**자동매매가 주문할 수 없는 종목을 조용히 실패시키지 않는다.** LS증권 확인(2026-09-14):
장외(OTC) 종목은 API 로 거래할 수 없고 **☎ 02-3779-8888 로 전화해 매도를 요청**해야 한다.
미국 정규장 시간에 LS 가 현지에서 주식을 사 갈 브로커를 찾아 매도해 준다.

### Fixed
- **모르는 거래소 코드를 나스닥으로 바꿔 보내던 조용한 폴백 제거** — 종전엔 매핑에 없는 코드를
  `.get(exchange, "82")` 로 나스닥으로 떨어뜨려 주문을 보냈고, 브로커가 `03053`("해당 종목번호가
  없습니다")로 거부했다. 사용자에게는 **종목 코드를 잘못 쓴 것처럼** 보여 진짜 원인이 가려졌다.
  이제 지원 밖 시장이면 **요청을 보내기 전에** 막고 사유를 남긴다 — 보내 봐야 성공할 수 없고
  LS 주문 속도 제한(초당 1건)만 쓴다.
  실측: 보유 종목 ZOMDF(잔고 거래소 `85`)를 허용값 `82`·`81` 로 각각 보내 **둘 다 03053 거부**.
  `OrdMktCode` 는 `Literal["81","82"]` 뿐이라 매핑을 고쳐도 해결되지 않는다 — 경로 자체가 없다.
- **신규·정정·취소 세 경로 모두** 같은 규칙을 적용. 취소가 특히 위험했다 — 조용히 나스닥으로
  바꾸면 **다른 시장의 같은 주문번호**를 취소하려 드는 셈이다.

### Added
- **체결 재조정기(fill reconciler)** — 놓친 체결을 뒤늦게 원장에 맞춰 넣는 주기 태스크(3분).
  종전엔 주문 접수 직후 인라인 확인 창이 4회 x 2초(실질 6초)뿐이라, 지정가가 몇 분 뒤에
  체결되면 그 사실이 **원장에 영영 도착하지 않았다**. 로트·체결내역은 체결 이벤트로만
  생기므로 `workflow_buy_amount` 가 0 이 되고, 분모가 0 이라 `workflow_pnl_rate` 가 NULL 로
  발행되어 **공유 전략 수익률이 통째로 비었다**(prod 실측 2026-09-14: 실제 체결된 매수·매도
  2건이 원장에 0건).
  - **대칭으로 만든다** — 자동매매로 산 게 아닐 수도, 판 게 아닐 수도 있다.
    ① 원장에 체결이 없는 우리 주문을 브로커에 확인해 넣는다.
    ② 로트는 남았는데 계좌엔 없는 종목(= 자동매매 **밖에서** 팔린 것으로 추정)을 보고한다.
    자동으로 로트를 지우지는 않는다 — 사라진 이유를 구분할 수 없어 **추정**이고, 자동 정리는
    사용자 수치를 근거 없이 바꾼다.
  - 매도 잔량 추정이 실제로 켜지도록 **계좌 평균매입가를 함께 넘긴다**. 그래서 "자동매매가
    사지 않은 물량을 팔았다" 가 `estimate_source=account_balance_avg_price` 로 남고,
    확정 실현손익(`realized_pnl`)에는 섞이지 않는다.
  - 브로커 조회는 **날짜당 1회**다(미확정이 몇 건이든). LS 주문체결내역 조회는 앱키당 2초
    1회이고 같은 앱키를 SDK 계좌 추적기가 60초 주기로 이미 쓴다. 잔고는 그 추적기 캐시에서
    읽어 **추가 조회가 0** 이다.
  - 안전장치: 접수 60초 이내 주문은 인라인 창과 겹쳐 제외(이중 계상 방지) · 브로커 응답에
    없는 주문은 미체결로 **단정하지 않음**(조회 실패·취소와 구분 불가) · 잔고를 못 읽으면
    빈 잔고가 아니라 **건너뜀**(보유 중인 로트를 전부 유령으로 기록하는 것을 막는다) ·
    한 주기가 터져도 루프는 살아남는다.

### Fixed
- **재조정의 종목 대조** — 주문번호는 계좌 안에서만 유일한데 원장에는 **계좌 교체 전 주문이
  그대로 남는다**(`workflow_orders` 에 계좌 컬럼이 없다). 브로커 응답의 종목이 원장과 다르면
  기록하지 않는다.
- **LS 응답 필드명** — 배치 조회가 종목을 `IsuNo` 로 읽고 있었다. 정본은 `ShtnIsuNo`
  (단축종목번호)이고 이 블록에는 **둘 다** 존재한다. `getattr` 기본값 때문에 오타가 예외
  없이 빈 값이 되어 위의 종목 대조 가드가 조용히 무력화됐다. 저장소 관행대로 ShtnIsuNo 우선 ·
  IsuNo 폴백으로 맞추고, 필드 실재를 단언하는 계약 테스트를 추가했다.

### Changed
- 차단 시 사용자 언어로 사유와 조치를 함께 낸다 — 어느 시장인지, 자동매매로는 불가하다는 사실,
  그리고 **LS증권 해외주식 데스크(02-3779-8888)** 연락처. 모듈 로거(`order_unsupported_market`)로
  파드 stdout 에 남고, 신규주문 경로는 투자자 알림(`ORDER_REJECTED`)까지 발행한다.
  구조화 진단은 `retry=do_not_retry` — 성공할 수 없는 요청을 재시도로 안내하지 않는다.
- 티커가 5자이고 `F` 로 끝나면 "장외로 넘어간 종목으로 보인다"는 힌트를 문구에 덧붙인다(LS 경험칙).
  ⚠️ LS 도 항상 맞는 규칙은 아니라고 했으므로 **차단 판단에는 쓰지 않는다** — 차단은 거래소 코드로만.
- deps: **core 1.28.1 필요** (`unsupported_market_reject` / `LS_OVERSEAS_DESK_PHONE` /
  `looks_like_otc_ticker` / `OVERSEAS_STOCK_ORDER_MARKET_CODES` 를 import 한다).
  finance 1.9.7 / community 1.15.3 은 변경 없음.

## [1.37.3] - 2026-09-14

1.37.2 를 prod 에 올린 직후 실관측으로 드러난 세 가지. **미국주식 주간거래(Blue Ocean) 세션은 지정가 주문만
받는다** — 시장가는 `rsp_cd=00891`("주간거래는 지정가로만 주문이 가능합니다")로 접수 자체가 거부된다(OrdNo 없음).

### Fixed
- **지정가 주문의 현재가 자동 조회를 매도에도 적용** — 종전엔 매수만 이 경로를 타서, 매도를 지정가로 두고
  가격을 비우면 `price=0` 이 그대로 나가 거부됐다. 손절 대상은 조건이 고르므로 사람이 종목마다 가격을 적을 수
  없다. 매수와 같은 규칙으로 맞춘다.
  ⚠️ 지정가는 **체결을 보장하지 않는다**(손절인데 안 팔릴 수 있다). 엔진이 주문 유형을 몰래 바꾸지는 않는다 —
  유형은 워크플로우 노드 설정에 그대로 드러나고, 그 선택의 결과다.
- **정상 무신호를 배선 고장으로 오판하던 거짓 경보 제거** — 상류가 **정상적으로** 빈 목록을 냈을 때
  (관심종목을 이미 보유 → 오늘 살 것 없음) 반복이 일어나지 않아 `{{ item.symbol }}` 이 리터럴로 남는데,
  1.37.2 가 그것을 `unresolved_template` = 고장으로 승격해 **매 실행 노드 FAILED + 사용자 알림**이 됐다.
  판정 축을 "반복 중인가" 에서 **"상류 배열이 있는데 비었는가"**(`_upstream_array_is_empty`)로 바꿨다 —
  리스트 포트가 아예 없으면 종전대로 배선 고장이다.

### Changed
- **주문 결과를 모듈 로거로도 남긴다**(`broker_order_response` / `broker_order_rejected` / `broker_order_submitted`).
  종전엔 전부 `context.log`(리스너 통지 전용)라 **파드 stdout 에 한 줄도 안 나왔고**, 접수됐는지 거부됐는지
  로그만으로는 구분할 수 없었다. 이제 `kubectl logs | grep broker_order_` 로 증권사 응답 코드가 보인다.
- deps: 변경 없음 (core 1.28.0 / finance 1.9.7 / community 1.15.3)

## [1.37.2] - 2026-09-14

조건 노드 하류의 주문이 **통과 종목이 아니라 평가 대상 전체를 순회**하던 결함. 2026-09-14 prod 실관측에서
손절 -8% 전략이 NIO −10.5% 에서도 매도 주문을 내지 못했다(증권사 주문내역 0건 · 미체결 0 · 체결 0 · 로그 0줄,
9/11 시작 이후 거래 0건). 같은 결함이 시계열 조건에서는 반대 방향으로 터진다 — 조건에 걸리지 않은 종목까지 주문된다.

### Fixed
- **auto-iterate 반복 대상 선택에 `passed_symbols` 승격 추가** — `ConditionNode` 는 통과 여부와 무관하게
  `symbols`(평가 대상 **전체**)를 함께 내보낸다. 보유기반 조건(StopLoss/ProfitTarget 등)에서 그 값은 종목코드
  **문자열 배열**이라 `{{ item.symbol }}`·`{{ item.exchange }}`·`{{ item.quantity }}` 가 모두 None 으로 풀려
  주문이 TR 호출 전에 버려졌고, 시계열 조건에서는 dict 라 **미통과 종목까지 주문**됐다.
  우선순위는 **명시 `from_port` > `symbols`(+`passed_symbols` 승격) > 소스 노드 첫 출력**.
  승격은 **선택된 소스 노드 안에서만** 한다 — 엣지를 가로지르면 ① 게이트로만 붙은 조건 노드가 진짜 반복
  소스(시계열)를 빼앗아 전략이 조용히 멈추고 ② `LogicNode` 하류 리터럴 주문이 1회 → N회가 되어 같은 주문이
  N번 나간다. 승격되면 **빈 배열이라도 정본**이라 통과 0건일 때 `symbols`(전체)로 흘러내리지 않는다.
- **반복 소스 선택이 엣지 선언 순서에 좌우되던 결함** — 첫 매칭 엣지에서 `break` 해, 계좌 엣지가 먼저 선언된
  워크플로우에서 사이징 노드가 **계좌 보유종목**을 순회했다(워크플로우에 없는 종목이 주문 후보가 된다).
  이제 incoming 엣지를 전부 훑어 우선순위로 고른다.
- **조건 통과 0건이면 주문 노드를 실행하지 않는다** — 종전엔 한 번 돌아 모든 아이템 바인딩이 None 인
  "종목 없는 주문" 을 만들었다. 스킵 출력은 **선언된 포트를 빈 값으로 채워** 하류의 포트 기대를 깨지 않고
  (`ConditionNode.result` 는 bool 유지), 주문 노드는 "오늘 주문 없음" 원장 행 1개를 남긴다.
- **주문 입력을 못 채웠을 때의 무음 종료 제거** — 어느 필드가 왜 비었는지와 반복 항목의 모양을 경고 로그
  (파드 stdout)·노드 출력(`blocked_orders` 배열 포트)·노드 실패 메시지에 남긴다. 종전엔 주문 결과가 전부
  `context.log`(리스너 전용, stdout 미출력)라 접수든 거부든 파드 로그가 똑같이 조용했다.
  단 **확실한 배선 고장**일 때만 실패로 승격한다 — 수량 0·자금 부족 같은 정상 무신호는 종전대로 `no_signal`
  이고, 진단이 상류 실패 사유(`fetch_failed` 등)를 가로채지 않도록 분류를 먼저 돌린다.
  국내주식은 거래소가 필수 필드가 아니다.

### Changed
- deps: 변경 없음 (core 1.28.0 / finance 1.9.7 / community 1.15.3 그대로)
...
- **토큰 provider 래퍼가 강제 재발급 신호를 전달한다** (`executor.LSClientManager`).
  ...
  stale_token=None)` 을 받는다. 이 신호가 서버까지 가지 않으면 서버는 만료 전 캐시 토큰을
  그대로 돌려주므로 재발급이 아무 효과가 없다. 주입된 provider 가 실제로 받는 키워드만
  골라 넘겨 **구판 3-인자 provider 와 호환**된다.

## [1.31.2] - 2026-08-19
> 선물 월물 해소·취소 경로 관측성 패치 — 모의실행이 만료 선물 심볼을 흘려 실주문 시세 노드를
> 죽이던 결함과, o3101/취소 실패가 원인을 숨기던 무음·추측 진단을 걷어낸다.
> 동반 릴리즈: `core` **1.24.0**(실주문 경로 3건 — 이 버전이 선행 조건이다),
> `finance` **1.8.0**(소스 무변경, lockstep), `community` 1.15.0(무변경).

### Fixed
- **dry_run 월물 fixture 만료 심볼 유출** — `deep_fixtures.futures_contract_fixture` 가
  `_FIXTURE_ANCHOR`(2025-01) 고정으로 월물을 산출해, 모의실행이 항상 19개월 전 만료 심볼
  (`HMHF25`)을 만들었다. 그 심볼이 하류 시세 노드로 흘러 만기 경과 종목으로 실제 LS 를 치면
  LS 는 시세도 과거봉도 주지 않고 에러도 안 내(빈 배열) 워크플로우가 조용히 죽었다. 월물 산출만
  실행 시각(오늘) 기준으로 전환하고(일봉·체결 fixture 는 재현성 위해 앵커 유지), HKEX 지수선물의
  월말 만기 구멍을 익월 롤오버로 막았다. 분기물 전용 상품의 원리적 한계는 라이브 노드의 사유
  노출로 '이름 있는 실패'가 되게 남겼다.
- **o3101 실패 진단이 추측이던 문제** — `FuturesContractNode` 가 빈 마스터 실패 시 `block` 만
  보고 `rsp_cd`/`rsp_msg`/`error_msg` 를 한 번도 안 읽어, 최종 메시지가 'entitlement 없거나
  앱키가 몰렸을 것'이라는 추측이었다. 업무거부는 HTTP 200 + 빈 block + `rsp_cd`(원인) 로 오고
  `error_msg` 는 빈 문자열일 수 있어 `if error_msg:` 로는 못 잡는다 → 매 시도의 LS 원문을 보관해
  재시도 경고와 최종 실패에 원문 우선으로 싣고, `rsp_cd` 로 '토큰 무효화 vs 시세 권한'을 분기해
  진단한다(라이브 프로브 불필요).
- **`OverseasFuturesSymbolQueryNode` 무음 빈 배열** — 거래소 가드가 `and rows` 조건 탓에 빈
  응답에서 건너뛰어지고 조용히 빈 배열을 반환했다(빈 유니버스 → 하류 전부 no-op → '성공'한 척
  무동작). 형제 노드와 같은 3회 재시도를 붙여 일시적 토큰 경합에 틱이 죽지 않게 하고, 그래도
  비면 `rsp_cd` 를 실어 하드 실패시킨다.
- **취소 3경로 빈-주문번호 가드 + 해외주식 AMEX 시장코드** — 원주문번호가 비어도 취소를 시도해
  브로커 거부를 유발하던 3경로에 가드를 추가하고, 해외주식 AMEX 주문의 시장코드를 교정했다.
- **주문 거부 진단이 원인을 말하게** — LS 업무거부의 실측 코드 7종(해외 4·국내 3)을 등재하고, `error_msg` 빈
  문자열을 '장 마감'으로 오진하던 분기 3곳을 `rsp_cd` 기준으로 교정했다.
- **국내 취소는 부분취소** — 취소 수량 미지정 시 1주만 취소되던 결함을 교정(CSPAT00801
  `OrdQty` = 취소할 수량, 해외의 전량 취소와 반대).
...
### Fixed
- **주문 실패가 completed 로 조용히 보고되던 결함(DEF-21)** — 주문 노드는 실패해도 raise 하지
  않고 `order_result.success=False` 를 돌려주는데, 엔진이 이를 COMPLETED(errors=0)로 보고해
  "주문이 안 나갔는데 모든 관측 표면이 성공으로 보이는" 자동매매 최악의 무징후 실패가 났다.
  이제 FAILED 노드 상태 + errors_count + last_error + 에러 로그로 표면화한다
  (`no_signal` = 정상 무매매는 실패 아님으로 제외).
- **死카운터 `orders_placed`/`orders_filled` 배선(DEF-26)** — 0 으로 초기화된 뒤 한 번도
  증가하지 않아 대시보드/모니터링이 실체결 후에도 "0건 주문"으로 표시했다. 접수(주문번호 수신)
  기준 placed, 재조회로 확인된 완전체결 기준 filled 로 각각 배선.
- **접수(order number) ≠ 체결 분리(DEF-27)** — 주문 TR 응답은 주문번호만 돌려주므로, 접수 직후
  상품별 재조회로 체결 여부를 best-effort 확인해 status 를 submitted → filled/partially_filled/open
  으로 승격(폴링 4×2s). 재조회 실패는 접수 성공을 뒤집지 않음. 재조회 TR: 국내주식 t0425,
  해외주식 COSAQ00102, 해외선물/옵션 CIDBQ02400.

### Changed
- deep_validate 시 주문 미발주를 blocking 구조 에러로 승격 — 챗봇이 주문 배선을 틀리면 build
  단계에서 실패하도록(런타임에 조용히 무발주 방지).
