...
전체 tool 목록이 LLM 에 그대로 전달되므로, 본 테스트에서는
tool_call.arguments 를 직접 stub 하여 pipe 연결성만 검증.
"""
...
class TestMarketStatusToolSchema:
    ...
    def test_tool_schema_has_expected_fields(self):
        """LLM 은 tool_name/description 을 기반으로 도구를 선택하므로
        핵심 키워드가 description 에 포함되어야 함."""
...
class TestMarketStatusAgentToolCallLoop:
    ...
    async def test_us_open_query_routes_to_market_status_tool(self):
        ...
        LLM 2차 호출에서 최종 답변 생성. 실제 LLM 의 도구 선택 로직은
