"""
ThrottleNode + RealMarketDataNode(GSC) 통합 테스트

해외주식 실시간 시세와 ThrottleNode의 skip/latest 모드 검증.
- 종목: AAPL, TSLA, NVDA, SOXL, QQQ (NASDAQ 활발한 종목)
- 테스트 시간: 미국장 개장 시간 (한국 23:30~06:00, 썸머타임 22:30~05:00)
"""
...
TEST_SYMBOLS = [
    ...
    {"exchange": "AMEX", "symbol": "SOXL"},
    {"exchange": "NASDAQ", "symbol": "QQQ"},
]

# 테스트 스킵 조건
skip_if_no_credentials = pytest.mark.skipif(
    not APPKEY or not APPSECRET,
    reason="APPKEY, APPSECRET 환경변수가 필요합니다"
)


def create_throttle_workflow(mode: str = "skip", interval_sec: float = 5.0) -> dict:
    """ThrottleNode 포함 워크플로우 생성"""
    return {
        "id": f"throttle-test-stock-{mode}",
        "name": f"Throttle {mode} 모드 테스트 - 해외주식",
        "description": f"RealMarketDataNode + ThrottleNode({mode}) 통합 테스트",
        "version": "1.0.0",
        "nodes": [
...
class TestThrottleWithGSC:
    """ThrottleNode + 해외주식 GSC 실시간 통합 테스트"""

    @skip_if_no_credentials
    @pytest.mark.asyncio
    async def test_skip_mode_with_real_ticks(self):
        """
        skip 모드: 빈번한 틱 무시 확인
        ...
        workflow = create_throttle_workflow(mode="skip", interval_sec=5.0)
        ...
        logger.info("=" * 60)
        logger.info("ThrottleNode SKIP 모드 테스트 시작 - 해외주식")
        ...
        try:
            ...
            logger.info("-" * 40)
            logger.info("테스트 결과 (SKIP 모드 - 해외주식):")
            ...
            if realmarket_outputs:
                ...
                logger.info(f"    - symbols: {len(symbols)}개")
                logger.info(f"    - prices: {dict(list(prices.items())[:3]) if prices else 'None'}...")
                
                if prices:
                    logger.info("✅ 실시간 시세 수신 확인 (장 시간 내)")
                else:
                    logger.info("ℹ️ 가격 데이터 없음 (장 시간 외)")
    ...
    @skip_if_no_credentials
    @pytest.mark.asyncio
    async def test_latest_mode_with_real_ticks(self):
        """
        latest 모드: 최신 틱만 유지 후 실행 확인
        ...
        workflow = create_throttle_workflow(mode="latest", interval_sec=5.0)
        ...
        logger.info("=" * 60)
        logger.info("ThrottleNode LATEST 모드 테스트 시작 - 해외주식")
        logger.info(f"interval_sec: 5초, 테스트 시간: 15초")
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.info("-" * 40)
            logger.info("테스트 결과 (LATEST 모드 - 해외주식):")
...
if __name__ == "__main__":
    ...
    async def main():
        test = TestThrottleWithGSC()
        
        print("\n" + "=" * 70)
        print("ThrottleNode + 해외주식 GSC 통합 테스트")
        print("=" * 70)
        
        if not APPKEY or not APPSECRET:
            print("❌ APPKEY, APPSECRET 환경변수가 필요합니다.")
            sys.exit(1)
        
        # skip 모드 테스트
        print("\n[1/2] SKIP 모드 테스트")
        await test.test_skip_mode_with_real_ticks()
        ...
        # latest 모드 테스트
        print("\n[2/2] LATEST 모드 테스트")
        await test.test_latest_mode_with_real_ticks()
