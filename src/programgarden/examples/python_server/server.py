...
# CORS 미들웨어 추가 (Flutter 웹에서 접근 허용)
app.add_middleware(
    CORSMiddleware,
...
# Workflow file-based APIs removed (use /api/workflow/run-inline instead)


# ========================================
# Flutter Widget Samples API
# ========================================

@app.get("/api/flutter/widget-samples")
async def get_flutter_widget_samples():
    """Flutter 앱에서 사용할 위젯 샘플 JSON 반환"""
    samples = {
        "widgets": [
            {
                "id": "dynamic_form",
                "title": "동적 폼 (Select → 필드 변경)",
                "type": "dynamic_form",
                "config": {
                    "nodeTypes": {
                        "http_request": {
                        ...
                        "database_query": {
                            ...
                            "fields": [
                                {"name": "connection", "type": "text", "label": "Connection String"},
                        ...
                        "file_operation": {
                            ...
                            "fields": [
                                ...
                                {"name": "operation", "type": "select", "label": "Operation", "options": ["read", "write", "delete"]},
                            ]
                        },
                    },
                    "defaultType": "http_request"
                }
            },
            {
                "id": "counter",
...
# /run/{workflow_id} endpoint removed (use /api/workflow/run-inline instead)
...
async def delete_file(source: str, filename: str):
    ...
    try:
        ...
        return JSONResponse({"error": str(e), "deleted": False}, status_code=400)
    except FileNotFoundError as e:
        return JSONResponse({"error": str(e), "deleted": False}, status_code=404)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse({"error": str(e), "deleted": False}, status_code=500)


# ========================================
# SQLite Database API (동적 테이블/컬럼 조회)
# ========================================

@app.get("/api/sqlite/{db_name}/tables")
async def list_sqlite_tables(db_name: str):
    """
    SQLite 데이터베이스의 테이블 목록 반환 (creatable_select 동적 옵션)
    
    Args:
        db_name: 데이터베이스 파일명 (예: "trading.db")
    ...
    try:
        ...
        return JSONResponse({"error": str(e), "tables": []}, status_code=400)
    except FileNotFoundError as e:
        return JSONResponse({"error": str(e), "tables": []}, status_code=404)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse({"error": str(e), "tables": []}, status_code=500)


@app.get("/api/sqlite/{db_name}/tables/{table_name}/columns")
async def list_sqlite_columns(db_name: str, table_name: str):
    """
    SQLite 테이블의 컬럼 목록 반환 (multi_select 동적 옵션)
    ...
    """
    try:
        from programgarden.tools.sqlite_tools import list_columns
        
        result = list_columns(db_name, table_name)
        return JSONResponse(result)
    except ValueError as e:
        return JSONResponse({"error": str(e), "columns": []}, status_code=400)
    except FileNotFoundError as e:
        return JSONResponse({"error": str(e), "columns": []}, status_code=404)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse({"error": str(e), "columns": []}, status_code=500)


@app.post("/api/sqlite/{db_name}/tables")
async def create_sqlite_table(db_name: str, request: Request):
    """
    SQLite 데이터베이스에 새 테이블 생성
    
    Args:
        db_name: 데이터베이스 파일명 (예: "trading.db")
        request body: {"table_name": "...", "columns": [{"name": "...", "type": "..."}]}
    
    Returns:
        생성 결과
    """
    try:
        from programgarden.tools.sqlite_tools import create_table
        ...
        if not table_name:
            return JSONResponse({"error": "table_name is required", "created": False}, status_code=400)
        if not columns:
            return JSONResponse({"error": "columns are required", "created": False}, status_code=400)
        
        result = create_table(db_name, table_name, columns)
        return JSONResponse(result)
    except ValueError as e:
        return JSONResponse({"error": str(e), "created": False}, status_code=400)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return JSONResponse({"error": str(e), "created": False}, status_code=500)
