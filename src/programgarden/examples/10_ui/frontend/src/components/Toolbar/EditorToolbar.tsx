...
export default function EditorToolbar() {
  ...
  // SSE connection for execution events
  useEffect(() => {
    if (!isRunning) return;

    const eventSource = new EventSource('/events');

    eventSource.addEventListener('node_state', (e) => {
      const data = JSON.parse(e.data);
      setNodeState(data.node_id, data.state);
      addLog({
        level: 'node',
        message: `Node [${data.node_id}] → ${data.state.toUpperCase()}`,
        nodeId: data.node_id,
      });
    });
    ...
    eventSource.onerror = () => {
      addLog({ level: 'warning', message: 'Event stream disconnected' });
    };

    return () => {
      eventSource.close();
    };
  }, [isRunning, setNodeState, setRunning, addLog]);
  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Convert to React Flow format if needed
        const nodes = (data.nodes || []).map((node: Record<string, unknown>) => ({
          id: node.id,
          type: 'customNode',
          position: node.position || { x: 0, y: 0 },
