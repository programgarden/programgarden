...
def get_workflow():
    return {
        ...
        "edges": [
            ...
            {"from": "macd.result", "to": "buyLogic.input2"},
            ...
            {"from": "buyLogic.passed_symbols", "to": "backtest.signals"},
            {"from": "historicalData.ohlcv_data", "to": "backtest.ohlcv_data"},
            {"from": "backtest.metrics", "to": "performanceCheck.performance_data"},
            {"from": "performanceCheck.passed", "to": "deploy.trigger"},
            {"from": "deploy.success", "to": "deployAlert.data"},
            {"from": "performanceCheck.failed", "to": "rejectAlert.data"},
            {"from": "deploy.result", "to": "display.data"},
        ],
    }
