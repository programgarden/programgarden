...
| 월물 만기 / Roll-over | 본 예제 분기물 J(4월) 사용. 만기 임박 roll-over 자동화는 예제 85 참조 |
...
flowchart LR
    ...
    hours --> watchlist[WatchlistNode<br/>HMHJ26, HMCEJ26]
...
| `watchlist` | 후보 종목 | HMHJ26, HMCEJ26 |
...
| `market_data` | 현재가 (auto-iterate per filtered symbol) | sizing 입력용 |
...
L3 (시세 read), L4 (mock 주문 1건) 는 사용자가 직접 모의 appkey 로 트리거합니다.
`/pg-test` 또는 `programmer_example/` 스크립트 활용.
