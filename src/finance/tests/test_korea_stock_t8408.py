"""Unit tests for t8408 업종차트(틱/n틱) (Korea Stock Sector Tick / N-tick Chart).

t8408 returns a tick / N-tick chart for one market sector / index. The response
is a two-block chart envelope (t8453-style ``cont_block`` / ``block`` naming):
    - ``cont_block`` (t8408OutBlock) — metadata + continuation cursor:
      previous-day / today's sector index OHLC, traded volume, the continuation
      cursors (``cts_date`` / ``cts_time``), session start/end times and the
      record count. Present only while more data is available.
    - ``block`` (t8408OutBlock1) — list of tick bars (one row per bar), each
      carrying the bar date/time, the bar index OHLC and the traded volume.

Guard coverage (per plan §4):
  Guard 1 — InBlock / OutBlock / OutBlock1 field SETS match the LS source
            (drift guard; INBLOCK=11, OUTBLOCK=16, OUTBLOCK1=7, so the t8453
            NXT/highend/lowend fields do not leak in).
  ...
  Guard 3 — chatbot-disambiguation: index OHLC fields are ``float`` and every
            ...
            fields are ``int`` (Guard 3b).
  Guard 4 — Request envelope: tr_cd='t8408', routes to
...
from programgarden_finance.ls.indtp.t8408 import TrT8408
from programgarden_finance.ls.indtp.t8408.blocks import (
    T8408InBlock,
    T8408OutBlock,
    T8408OutBlock1,
    T8408Request,
...
# ---------------------------------------------------------------------------
# LS source field sets (verbatim from the t8408 spec)
# ---------------------------------------------------------------------------
...
OUTBLOCK_FIELDS = {
    ...
    "disiga", "dihigh", "dilow", "diclose", "cts_date", "cts_time",
    "s_time", "e_time", "dshmin", "rec_count",
}
OUTBLOCK1_FIELDS = {"date", "time", "open", "high", "low", "close", "jdiff_vol"}
...
# Official LS REST sample response (shcode '001', 20230605)
LS_SAMPLE = {
    "t8408OutBlock": {
        "shcode": "001",
        ...
        "diclose": "2610.85",
        "cts_date": "20230605", "cts_time": "102650",
        "s_time": "090000", "e_time": "153000", "dshmin": "10", "rec_count": 1,
    },
    "rsp_cd": "00000",
    "rsp_msg": "정상적으로 조회가 완료되었습니다.",
    "t8408OutBlock1": [{
        "date": "20230605", "time": "102700",
        "open": "2610.85", "high": "2610.85", "low": "2610.85",
        "close": "2610.85", "jdiff_vol": 215,
    }],
}

RESP_HEADERS = {
    "Content-Type": "application/json; charset=utf-8",
    "tr_cd": "t8408", "tr_cont": "N", "tr_cont_key": "",
}
...
def _make_inblock(**overrides) -> T8408InBlock:
    base = dict(shcode="001", ncnt=1, qrycnt=1, nday="0", edate="99999999")
    base.update(overrides)
    return T8408InBlock(**base)


def _make_tr(**overrides) -> TrT8408:
    return TrT8408(T8408Request(body={"t8408InBlock": _make_inblock(**overrides)}))
...
def test_inblock_field_set():
    assert set(T8408InBlock.model_fields) == INBLOCK_FIELDS
    assert len(INBLOCK_FIELDS) == 11  # LS source declares exactly 11


def test_outblock_field_set():
    assert set(T8408OutBlock.model_fields) == OUTBLOCK_FIELDS
    # exactly 16 — guards against t8453's NXT/highend/lowend leaking in
    assert len(OUTBLOCK_FIELDS) == 16


def test_outblock1_field_set():
    assert set(T8408OutBlock1.model_fields) == OUTBLOCK1_FIELDS
    assert len(OUTBLOCK1_FIELDS) == 7  # LS source declares exactly 7
...
def test_outblock_index_ohlc_fields_are_float(field_name):
    assert T8408OutBlock.model_fields[field_name].annotation is float
...
def test_outblock1_index_ohlc_fields_are_float(field_name):
    assert T8408OutBlock1.model_fields[field_name].annotation is float


@pytest.mark.parametrize("field_name", sorted(OUTBLOCK_INDEX_OHLC_FIELDS))
def test_outblock_index_ohlc_described_as_index_points_not_price(field_name):
    desc = (T8408OutBlock.model_fields[field_name].description or "").lower()
...
def test_outblock1_index_ohlc_described_as_index_points_not_price(field_name):
    desc = (T8408OutBlock1.model_fields[field_name].description or "").lower()
    assert "index points" in desc, f"{field_name} must say it is index points"
    assert "not a" in desc and "price" in desc, (
        f"{field_name} must say it is NOT a price (index vs price disambiguation)"
    )
...
# ===========================================================================
# Guard 3b — volume / count fields are int
# ===========================================================================


def test_volume_and_count_fields_are_int():
    assert T8408OutBlock.model_fields["jivolume"].annotation is int
    assert T8408OutBlock.model_fields["rec_count"].annotation is int
    assert T8408OutBlock1.model_fields["jdiff_vol"].annotation is int
...
def test_request_envelope():
    req = T8408Request(body={"t8408InBlock": _make_inblock()})
    assert req.header.tr_cd == "t8408"
    assert req.body["t8408InBlock"].shcode == "001"
    # LS-source-declared defaults
    ib = req.body["t8408InBlock"]
    assert ib.comp_yn == "N"
    assert ib.sdate == " "
...
def test_comp_yn_rejects_invalid():
    with pytest.raises(ValidationError):
        T8408InBlock(shcode="001", ncnt=1, qrycnt=1, nday="0", edate="99999999", comp_yn="X")
...
def test_build_response_success_parses_sample():
    ...
    # volume / count stay int
    assert isinstance(cb.jivolume, int) and cb.jivolume == 569620
    assert isinstance(cb.rec_count, int) and cb.rec_count == 1

    # block rows
    assert len(resp.block) == 1
    row = resp.block[0]
    assert row.date == "20230605" and row.time == "102700"
    assert isinstance(row.close, float) and row.close == pytest.approx(2610.85)
    assert row.open == pytest.approx(2610.85)
    assert isinstance(row.jdiff_vol, int) and row.jdiff_vol == 215
...
def test_terminal_page_cont_block_is_none():
    """Terminal page: LS omits ``t8408OutBlock`` entirely when there is no more
    data → ``cont_block`` must be None while the row block still parses."""
    tr = _make_tr()
    sample = {k: v for k, v in LS_SAMPLE.items() if k != "t8408OutBlock"}
    resp = tr._build_response(_resp(200), sample, RESP_HEADERS, None)
    assert resp.cont_block is None
    assert resp.error_msg is None
    assert len(resp.block) == 1  # rows still parsed on the final page
...
def test_occurs_continuation_feeds_cts():
    ...
    def fake_occurs(updater, callback=None, delay=1):
        updater(req, resp)
        captured["cts_date"] = req.body["t8408InBlock"].cts_date
        captured["cts_time"] = req.body["t8408InBlock"].cts_time
    ...
    assert captured["cts_time"] == "102650"
...
def test_rate_limit_config():
    ...
    assert req.options.rate_limit_key == "t8408"
...
def test_response_envelope_signature():
    for f in ("header", "cont_block", "block", "status_code", "rsp_cd", "rsp_msg", "error_msg"):
        assert f in T8408Response.model_fields
