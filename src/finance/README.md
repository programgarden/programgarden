...
- **업종(indtp)**: 시세 `t1511`(업종현재가), `t1514`(업종기간별추이), `t1516`(업종별종목시세)(`/indtp/market-data`) + 차트 `t8408`(업종차트틱)(`/indtp/chart`) — `ls.indtp()`/`ls.업종()`으로 이전
...
from programgarden_finance import (
    ...
    t1511, t1514, t1516, t8408,                 # 업종(indtp, ls.업종())
