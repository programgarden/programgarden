...
> KRX 프리/애프터마켓 시간대가 응답에 실려 와도 **파싱 단계에서 조용히 버려진다**
> (pydantic 기본 동작이 미정의 키 무시라 오류조차 나지 않는다).

### Added
- **`exchgubun`(거래소구분코드) InBlock 추가** — `t1901` / `t1903` / `t1904`.
  `'K'` = KRX(한국거래소) / `'N'` = NXT(넥스트레이드) / `'U'` = 통합. 기본값 `'K'` 로,
  종전 동작(KRX 조회)이 그대로 유지된다. `t8451` / `t8452` / `t8453` 은 이미 갖고 있어
  변경 없음.
- **`ex_shcode`(거래소별종목코드) OutBlock 추가** — `t1901OutBlock`(공지 라벨은
  거래소별단축코드) · `t1903OutBlock` · `t1904OutBlock` · `t1904OutBlock1`.
- **`nxt_vi_gubun`(NXTVI발동해제) `t1901OutBlock` 추가** — 기존 `vi_gubun` 의 NXT 대응
  필드. 코드값은 공지·가용 소스에 정의가 없어 LS 가 준 값 그대로 소비한다.
- **KRX 프리/애프터마켓 시간대 6종 OutBlock 추가** — `t8451` / `t8452` / `t8453`:
  `krx_fm_s_time` · `krx_fm_e_time` · `krx_fm_dshmin` ·
  `krx_am_s_time` · `krx_am_e_time` · `krx_am_dshmin`.
  같은 블록의 기존 `nxt_*` 6종과 짝을 이룬다(시각 `'HHMMSS'`, 동시호가 처리시간 분 `'MM'`).

### Notes
- **`t1902`(ETF시간별추이) · `t1906`(ETF LP호가) 는 이 라이브러리에 미구현**이라 공지의
  해당 항목(각 InBlock `exchgubun`, OutBlock `ex_shcode`, t1906 의 NXT 예상체결·중간가
  9종)은 반영 대상이 없다. 두 TR 을 신규 추가하려면 공지의 "추가 필드" 목록만으로는
  부족하고 전체 블록 정의가 필요하다.
- 신규 OutBlock 필드는 전부 기본값이 있어, **적용일(2026-09-12) 이전의 응답**에도
  그대로 파싱된다. InBlock 의 `exchgubun` 은 기본값 `'K'` 가 항상 전송되지만 이는
  `t8451` 계열이 이미 하던 동작과 같다.

## [1.9.1] - 2026-08-20
> LS 게이트웨이 토큰 실패 코드의 **형제 코드**를 등재한다. 표에 없으면 토큰 문제가
> "알 수 없는 오류" 로 흘러 재발급 경로를 못 탄다.
> 동반 릴리즈: `core` **1.25.0** · `programgarden` **1.33.0**.

### Added
- `TOKEN_RSP_CODES` 확장 — `IGW00122` / `IGW00123` + session_key 계열 3종.
  종전엔 라이브 실측값 `IGW00121` 하나뿐이었다. LS 개발자센터가 로그인 뒤에 있어 공식
  코드표를 볼 수 없었고, 추측 코드를 넣으면 주문 거부를 토큰 문제로 오판하므로
  **"실측값만 등재"** 규율을 지켜 왔다. 2026-08-19 확보한 DB증권 공개 표가 우리 LS 실측
  3건과 코드·메시지 모두 일치해 근거가 생겼다.
...
### Changed
- **동기/비동기 갱신 경로가 서로 배타적이 됐다.** 같은 TokenManager 를 동기 경로는
  `threading.Lock`, 비동기 경로는 `asyncio.Lock` 으로 각각 보호해 **서로를 막지 못했다** —
  둘이 동시에 재발급하면 토큰이 두 번 발급돼 이 릴리즈가 없애려는 "1 앱키 = 1 토큰"을
  스스로 깬다. 비동기 경로가 async 락 안에서 동기 락도 함께 잡는다(잠금 순서 단방향,
  acquire 는 스레드로 넘겨 이벤트 루프를 세우지 않는다).
- **재발급 상한이 '시도'가 아니라 '성공'을 센다.** 종전에는 발급 호출 **전에** 기록해서,
  회선 장애로 실패한 시도가 예산을 다 태웠다 — 몇 분 뒤 찾아온 진짜 토큰 사고를 막지 못하고
  "앱키 권한을 확인하세요"라는 오진 메시지로 실패했다.
- **네트워크 예외는 강제 재발급으로 승격하지 않는다.** 서버에 닿지도 못한 실패는 토큰을
  바꿀 이유가 없다(간격 재시도만 한다).
