"""KOSDAQ 호가잔량(HA_) 실시간 WebSocket 요청/응답 모델

EN:
    Pydantic models for the HA_ (KOSDAQ Order Book) real-time WebSocket stream.
    Provides real-time 10-level order book data for KOSDAQ-listed stocks.
    Field structure is identical to H1_ (KOSPI).

KO:
    KOSDAQ 종목의 실시간 호가잔량(10단계 매도/매수 호가) 데이터를 수신하기 위한
    WebSocket 요청/응답 모델입니다. 필드 구조는 H1_(KOSPI 호가잔량)와 동일합니다.
"""
...
class HA_RealRequestBody(BaseModel):
    tr_cd: str = Field("HA_", description="거래 CD")
    tr_key: str = Field(..., max_length=8, description="KOSDAQ 종목 단축코드 6자리 또는 8자리 (예: '086520')")
...
class HA_RealRequest(BaseModel):
    ...
    EN:
        WebSocket subscription request for KOSDAQ real-time order book data.
        Use tr_type='3' to subscribe, '4' to unsubscribe.

    KO:
        KOSDAQ 종목의 실시간 호가잔량 데이터 수신을 위한 WebSocket 등록/해제 요청입니다.
...
class HA_RealResponseBody(BaseModel):
    """KOSDAQ 호가잔량(HA_) 실시간 응답 바디

    EN:
        Real-time KOSDAQ 10-level order book data body.
        Field structure is identical to H1_ (KOSPI Order Book).

    KO:
        KOSDAQ 종목의 실시간 10단계 호가잔량 데이터 바디입니다.
        필드 구조는 H1_(KOSPI 호가잔량)와 동일합니다.
    """
    hotime: str = Field(..., title="호가시간", description="호가 수신 시간 (HHMMSS)")
    """호가시간"""
    ...
    """총매수호가잔량"""
    donsigubun: str = Field(..., title="동시호가구분", description="동시호가구분")
    """동시호가구분"""
    shcode: str = Field(..., title="단축코드", description="종목 단축코드 6자리 (예: '086520')")
    """단축코드"""
    ...
    """매수중간가잔량합계수량"""
    midsumrem: str = Field(..., title="중간가잔량합계수량", description="중간가 잔량 합계")
    """중간가잔량합계수량"""
...
class HA_RealResponse(BaseModel):
    ...
    EN:
        Complete response model for HA_ real-time KOSDAQ order book data.

    KO:
        KOSDAQ 호가잔량 실시간 데이터의 전체 응답 모델입니다.
    """
