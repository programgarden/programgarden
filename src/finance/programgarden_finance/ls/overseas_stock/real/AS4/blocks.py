"""Pydantic models for LS Securities OpenAPI AS4 (Overseas Stock Order Reject — US Markets).

AS4 is a Real-time WebSocket TR that pushes order-reject events for US-listed
overseas stocks. The ``AS4RealRequestBody`` carries only the WebSocket
subscription envelope (``tr_cd`` + ``tr_key``); the ``AS4RealResponseBody``
carries the per-reject push payload — a system / WS frame header followed by
reject-related fields.

Note vs. AS1~AS3: the AS4 push payload truncates the AS1-style trailing
balance fields after ``sCsgnMnyMgn`` — ``sCsgnSubstMgn``, ``sOrdAbleMny``,
``sOrdAbleSubstAmt``, ``sRuseAbleAmt`` and ``sMgntrnCode`` are not part of
the AS4 reject payload.
...
2026-05-06 finance TR field metadata plan):
    ...
    - Enum codes documented verbatim (``sOrdxctPtnCode``, ``sOrdPtnCode``)
      are listed exhaustively. Code classifiers without a declared enum
      mapping in the source are described as "consume as returned by LS."
    ...
    - ``sBnsTp`` enum is NOT documented in the AS4 source's in-line comments
      (unlike AS1) — described as "consume as returned by LS." rather than
      inferred from AS1.
"""
...
class AS4RealRequestHeader(BlockRealRequestHeader):
    """AS4 real-time request header. Inherits the standard LS WS request header schema."""
    pass


class AS4RealResponseHeader(BlockRealResponseHeader):
    """AS4 real-time response header. Inherits the standard LS WS response header schema."""
    pass


class AS4RealRequestBody(BaseModel):
    """AS4RealRequestBody — WebSocket subscription envelope for reject push."""

    tr_cd: str = Field(
        default="AS4",
        title="거래 CD (TR code)",
        description="Fixed TR code identifier for this subscription. Always 'AS4'.",
        examples=["AS4"],
    )
...
class AS4RealRequest(BaseModel):
    """
    해외주식주문거부(미국) 실시간 요청 (Overseas Stock Order Reject — US, real-time request envelope).
    """
    header: AS4RealRequestHeader = Field(
        ...
        description="AS4 WebSocket subscription header block (token + tr_type)."
    )
...
class AS4RealResponseBody(BaseModel):
    """AS4RealResponseBody — reject push payload for a US overseas-stock symbol."""
    ...
    sOrdNo: int = Field(
        ...
        description="Order number this event applies to.",
        examples=[231, 100001],
    )
    sOrgOrdNo: int = Field(
        ...,
        title="원주문번호 (Original order number)",
        description="Original order number when applicable (0 otherwise).",
        examples=[0, 231],
    )
    sExecNO: str = Field(
        ...,
        title="체결번호 (Execution number)",
        description="LS-assigned execution / fill number (when applicable).",
        examples=[""],
    )
    ...
    sOrdAmt: float = Field(
        ...
        description="Order amount before fees.",
        examples=[0.0, 180.5],
    )
    sStdIsuNo: str = Field(
        ...,
        title="표준종목번호 (Standard issue code)",
        description="Standardised issue code.",
    ...
    sMnyExecAmt: float = Field(
        ...
        description="Cash component of any executed amount.",
        examples=[0.0],
    )
    sSubstExecAmt: float = Field(
        ...,
        title="대용체결금액 (Substitute execution amount)",
        description="Substitute-collateral component of any executed amount.",
        examples=[0.0],
    )
    ...
    sRcptExecTime: str = Field(
        ...
        description="Time stamp at which the exchange-confirmed event was received.",
        examples=["093015"],
    )
    sRjtRsn: str = Field(
        ...,
        ...
        description=(
            "Reject reason text / code. Format not declared in available source — "
