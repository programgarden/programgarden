"""Pydantic models for LS Securities OpenAPI AS3 (Overseas Stock Order Cancel — US Markets).

AS3 is a Real-time WebSocket TR that pushes order-cancel events for US-listed
overseas stocks. The ``AS3RealRequestBody`` carries only the WebSocket
subscription envelope (``tr_cd`` + ``tr_key``); the ``AS3RealResponseBody``
carries the per-cancel push payload — a system / WS frame header followed by
cancel-related fields (mirrors AS1/AS2 shape).
...
2026-05-06 finance TR field metadata plan):
    ...
    - ``sBnsTp`` enum is NOT documented in the AS3 source's in-line comments
...
class AS3RealRequestHeader(BlockRealRequestHeader):
    """AS3 real-time request header. Inherits the standard LS WS request header schema."""
    pass


class AS3RealResponseHeader(BlockRealResponseHeader):
    """AS3 real-time response header. Inherits the standard LS WS response header schema."""
    pass


class AS3RealRequestBody(BaseModel):
    """AS3RealRequestBody — WebSocket subscription envelope for cancel push."""

    tr_cd: str = Field(
        default="AS3",
        title="거래 CD (TR code)",
        description="Fixed TR code identifier for this subscription. Always 'AS3'.",
        examples=["AS3"],
    )
...
class AS3RealRequest(BaseModel):
    """
    해외주식주문취소(미국) 실시간 요청 (Overseas Stock Order Cancel — US, real-time request envelope).
    """
    header: AS3RealRequestHeader = Field(
        ...
        description="AS3 WebSocket subscription header block (token + tr_type)."
    )
...
class AS3RealResponseBody(BaseModel):
    """AS3RealResponseBody — cancel push payload for a US overseas-stock symbol."""
    ...
    sOrgOrdNo: int = Field(
        ...
        description="Original order number being cancelled.",
        examples=[231],
    )
    sExecNO: str = Field(
        ...,
