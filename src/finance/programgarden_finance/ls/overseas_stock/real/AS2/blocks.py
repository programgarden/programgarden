"""Pydantic models for LS Securities OpenAPI AS2 (Overseas Stock Order Modify — US Markets).

AS2 is a Real-time WebSocket TR that pushes order-modify events for US-listed
overseas stocks. The ``AS2RealRequestBody`` carries only the WebSocket
subscription envelope (``tr_cd`` + ``tr_key``); the ``AS2RealResponseBody``
carries the per-modify push payload — a system / WS frame header followed by
modify-specific fields (mirrors AS1's execution shape).
...
2026-05-06 finance TR field metadata plan):
    ...
    - ``sBnsTp`` enum is NOT documented in the AS2 source's in-line comments
...
class AS2RealRequestHeader(BlockRealRequestHeader):
    """AS2 real-time request header. Inherits the standard LS WS request header schema."""
    pass


class AS2RealResponseHeader(BlockRealResponseHeader):
    """AS2 real-time response header. Inherits the standard LS WS response header schema."""
    pass


class AS2RealRequestBody(BaseModel):
    """AS2RealRequestBody — WebSocket subscription envelope for modify push."""

    tr_cd: str = Field(
        default="AS2",
        title="거래 CD (TR code)",
        description="Fixed TR code identifier for this subscription. Always 'AS2'.",
        examples=["AS2"],
    )
...
class AS2RealRequest(BaseModel):
    """
    해외주식주문정정(미국) 실시간 요청 (Overseas Stock Order Modify — US, real-time request envelope).
    """
    header: AS2RealRequestHeader = Field(
        ...
        description="AS2 WebSocket subscription header block (token + tr_type)."
    )
...
class AS2RealResponseBody(BaseModel):
    """AS2RealResponseBody — modify push payload for a US overseas-stock symbol."""
    ...
    sOrgOrdNo: int = Field(
        ...
        description="Original order number being modified.",
        examples=[231],
    )
    sExecNO: str = Field(
        ...,
