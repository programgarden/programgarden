...
logger = logging.getLogger("programgarden.ls.overseas_stock.order.COSAT00400")
from .blocks import (
    COSAT00400InBlock1,
    COSAT00400OutBlock1,
    COSAT00400OutBlock2,
...
class TrCOSAT00400(TROrderAbstract):
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> COSAT00400Response:
        ...
        if exc is not None:
            error_msg = str(exc)
            logger.error(f"COSAT00400 request failed: {exc}")
        elif is_error_status:
            error_msg = f"HTTP {status}"
            if resp_json.get("rsp_msg"):
                error_msg = f"{error_msg}: {resp_json['rsp_msg']}"
            logger.error(f"COSAT00400 request failed with status: {error_msg}")
