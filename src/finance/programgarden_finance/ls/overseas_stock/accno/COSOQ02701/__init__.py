...
logger = logging.getLogger("programgarden.ls.overseas_stock.accno.COSOQ02701")
...
class TrCOSOQ02701(TRAccnoAbstract):
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> COSOQ02701Response:
        ...
        if exc is not None:
            error_msg = str(exc)
            logger.error(f"COSOQ02701 request failed: {exc}")
        elif is_error_status:
            error_msg = f"HTTP {status}"
            if resp_json.get("rsp_msg"):
                error_msg = f"{error_msg}: {resp_json['rsp_msg']}"
            logger.error(f"COSOQ02701 request failed with status: {error_msg}")
