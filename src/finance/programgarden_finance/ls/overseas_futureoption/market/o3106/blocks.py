"""Pydantic models for LS Securities OpenAPI o3106 (Overseas futures current price + 5-level orderbook).

o3106 returns a single-block snapshot combining the latest market data and a
5-level bid/ask orderbook for one overseas futures contract.
...
2026-05-06 finance TR field metadata plan):
    ...
    - Per-level quantity notes: no special constraint (e.g., "level 2..10
      quantities always 0") is declared for overseas-futures o3106 in the
      available source. Each level is documented as standard orderbook semantics.
    - ``examples`` come from ``src/finance/example/overseas_futureoption/run_o3106.py``
...
class O3106RequestHeader(BlockRequestHeader):
    """o3106 request header. Inherits the standard LS request header schema."""
    pass


class O3106ResponseHeader(BlockResponseHeader):
    """o3106 response header. Inherits the standard LS response header schema."""
    pass


class O3106InBlock(BaseModel):
    """o3106InBlock — input block for the overseas futures current price + orderbook query."""
...
class O3106Request(BaseModel):
    """o3106 full request envelope (header + body + setup options)."""

    header: O3106RequestHeader = Field(
        O3106RequestHeader(
    ...
    body: Dict[Literal["o3106InBlock"], O3106InBlock] = Field(
        ...
        description="Wrapped input block keyed by 'o3106InBlock'.",
    )
...
class O3106OutBlock(BaseModel):
    """o3106OutBlock — current price and 5-level bid/ask orderbook snapshot.
