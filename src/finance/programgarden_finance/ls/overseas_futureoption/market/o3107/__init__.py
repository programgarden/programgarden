...
logger = logging.getLogger("programgarden.ls.overseas_futureoption.market.o3107")
...
class TrO3107(TRRequestAbstract):
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> O3107Response:
        ...
        if exc is not None:
            error_msg = str(exc)
            logger.error(f"o3107 request failed: {exc}")
        elif is_error_status:
            error_msg = f"HTTP {status}"
            if resp_json.get("rsp_msg"):
                error_msg = f"{error_msg}: {resp_json['rsp_msg']}"
            logger.error(f"o3107 request failed with status: {error_msg}")
