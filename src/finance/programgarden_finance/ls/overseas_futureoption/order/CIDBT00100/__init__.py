...
logger = logging.getLogger("programgarden.ls.overseas_futureoption.order.CIDBT00100")
...
class TrCIDBT00100(TROrderAbstract):
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> CIDBT00100Response:
        ...
        if exc is not None:
            error_msg = str(exc)
            logger.error(f"CIDBT00100 request failed: {exc}")
        elif is_error_status:
            error_msg = f"HTTP {status}"
            if resp_json.get("rsp_msg"):
                error_msg = f"{error_msg}: {resp_json['rsp_msg']}"
            logger.error(f"CIDBT00100 request failed with status: {error_msg}")
