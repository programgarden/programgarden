"""Pydantic models for LS Securities OpenAPI o3139 (Overseas futures/options tick chart by market type).

o3139 retrieves tick-based chart data for a given overseas futures/options
symbol with explicit market-type selection (futures / options). The response
carries:
...
2026-05-06 finance TR field metadata plan):
    ...
    - ``examples`` come from ``src/finance/example/overseas_futureoption/run_o3139.py``
      where present.
"""
...
class O3139RequestHeader(BlockRequestHeader):
    """o3139 request header. Inherits the standard LS request header schema."""
    pass


class O3139ResponseHeader(BlockResponseHeader):
    """o3139 response header. Inherits the standard LS response header schema."""
    pass


class O3139InBlock(BaseModel):
    """o3139InBlock — input block for the tick chart query with market-type selection."""

    mktgb: str = Field(
        ...,
        title="시장구분 (Market type)",
        description=(
            "Market type code. 'F' = futures (선물), 'O' = options (옵션). "
    ...
    ncnt: int = Field(
        ...
        title="단위 (Tick unit)",
        description=(
            "Tick aggregation unit (number). Other LS-defined values may appear; "
...
class O3139Request(BaseModel):
    """o3139 full request envelope (header + body + setup options)."""

    header: O3139RequestHeader = Field(
        O3139RequestHeader(
    ...
    body: dict[Literal["o3139InBlock"], O3139InBlock] = Field(
        ...,
        title="입력 데이터 블록 (Input body)",
        description="Wrapped input block keyed by 'o3139InBlock'.",
    )
...
class O3139OutBlock(BaseModel):
    """o3139OutBlock — echoed request parameters and server-side metadata.

    LS echoes the request inputs back in OutBlock along with the record count,
    continuation keys for paged retrieval, and last-tick count. The actual
    tick-bar rows live in ``OutBlock1``.
    """

    shcode: Optional[str] = Field(
        default=None,
        title="단축코드 (Short code / symbol)",
        description="Echoed overseas futures/options short code (e.g., 'CUSU25').",
        examples=["CUSU25", None],
    )
    ...
    cts_seq: Optional[str] = Field(
        default=None,
        title="연속시간(순번CTS) (Continuation sequence)",
        description="Continuation sequence key for the next paged request.",
        examples=["", "00001234", None],
    )
    cts_daygb: Optional[str] = Field(
        default=None,
        title="연속당일구분 (Continuation day flag)",
        description="Continuation day flag for the next paged request.",
        examples=["", "0", None],
    )
    last_count: Optional[int] = Field(
        default=None,
        title="마지막Tick건수 (Last-tick count)",
        description=(
            "Last-tick count reported by LS. Meaning not further declared "
