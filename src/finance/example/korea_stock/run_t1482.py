...
import os
from programgarden_finance import LS, t1482
...
def test_req_t1482():
    """t1482 시간외거래량상위 테스트 (장 종료 후 데이터 존재)"""
    ...
    # 거래량순(0) 전체시장(0) 전체종목(0) 조회
    m = ls.국내주식().순위().시간외거래량상위(
        t1482.T1482InBlock(sort_gbn=0, gubun="0", jongchk="0")
    )
