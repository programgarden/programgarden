...
import os
from programgarden_finance import LS, t8408
...
def test_req_t8408():
    ...
    # 코스피 종합(001) 업종의 1틱 차트 최근 1건.
    # 신경로: ls.indtp().업종차트틱(...) (= ls.업종().t8408(...)).
    m = ls.indtp().업종차트틱(
        t8408.T8408InBlock(
    ...
    if response.cont_block:
        ...
        logger.info(
            ...
            f"저:{cb.jilow:,.2f} 종:{cb.jiclose:,.2f} | 거래량:{cb.jivolume:,}"
        )
        logger.info(
            f"  당일 지수 OHLC  시:{cb.disiga:,.2f} 고:{cb.dihigh:,.2f} "
            f"저:{cb.dilow:,.2f} 종:{cb.diclose:,.2f}"
        )
        logger.info(f"  장 시작:{cb.s_time} 종료:{cb.e_time}")
        if cb.cts_date or cb.cts_time:
            logger.info(f"  연속커서 cts_date:{cb.cts_date} cts_time:{cb.cts_time}")

    logger.info(f"조회 틱 행 수: {len(response.block)}건")
    logger.info("=" * 90)
    logger.info(
        f"{'일자':<10} {'시간':<8} {'시가':>10} {'고가':>10} "
        f"{'저가':>10} {'종가':>10} {'거래량':>12}"
    )
    logger.info("-" * 90)
    ...
    for row in response.block:
        ...
        logger.info(
            ...
            f"{row.low:>10,.2f} {row.close:>10,.2f} {row.jdiff_vol:>12,}"
        )


def test_occurs_req_t8408():
    """t8408 연속조회 예제 — cts_date/cts_time 기반 대량 틱 차트 수집."""
