...
| `pluck(list, path)` | 배열에서 특정 경로 값 추출 | `{{ pluck(items, "name") }}` → `["AAPL", "TSLA"]` |
| `flatten(list, key)` | 중첩 배열 평탄화 (부모 필드 유지) | 아래 예시 참조 |
...
| 함수 | 결과 | 설명 |
|------|------|------|
| `{{ pluck(values, "symbol") }}` | `["AAPL", "TSLA"]` | 특정 키만 추출 |
| `{{ flatten(values, "time_series") }}` | `[{"symbol": "AAPL", "date": "20251224", "rsi": 33.5}, ...]` | 부모 필드 유지하며 평탄화 |

> **팁**: `flatten`은 차트 노드에 데이터를 전달할 때 유용합니다. 종목별 시계열 데이터를 하나의 배열로 합칩니다.
...
```
{{ pluck(positions, "details.sector") }}   →   ["Tech", "Auto", "Finance"]
```
...
```json
{
  ...
  "title": "포트폴리오 현황",
  "items": [
    {"label": "수익률", "value": "{{ format_pct(nodes.account.pnl_rate, 2) }}"},
    {"label": "총 자산", "value": "{{ format_currency(nodes.account.total_eval) }}"}
  ]
}
```
...
| `"$input.rsi_period"` | `"{{ input.rsi_period }}"` |
