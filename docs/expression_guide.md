...
# pluck: 특정 키만 추출 (부모 정보 손실)
{{ pluck(values, "symbol") }}
# → ["AAPL", "TSLA"]

{{ pluck(values, "time_series") }}
# → [[{"date": "20251224", "rsi": 33.5}], [{"date": "20251224", "rsi": 62.1}]]

# flatten: 부모 필드 유지하며 평탄화 (multi_line 차트에 적합)
{{ flatten(values, "time_series") }}
# → [
...
```python
# 점 표기법으로 깊은 경로 접근
{{ pluck(positions, "details.sector") }}
# → ["Tech", "Auto", "Finance"]
```
