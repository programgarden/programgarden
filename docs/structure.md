...
51개 노드가 11개 카테고리로 분류되어 있습니다.

| 카테고리 | 용도 | 주요 노드 |
|----------|------|----------|
| `infra` | 워크플로우 시작, 증권사 연결, 흐름 제어 | StartNode, BrokerNode, ThrottleNode, SplitNode, AggregateNode |
| `account` | 잔고, 포지션, 미체결 주문 | AccountNode, OpenOrdersNode, RealAccountNode, RealOrderEventNode |
| `market` | 시세, 종목 목록, 과거 데이터 | WatchlistNode, MarketDataNode, HistoricalDataNode, RealMarketDataNode, ScreenerNode |
...
| `data` | DB 저장/조회, 외부 API, 필드 매핑 | SQLiteNode, HTTPRequestNode, FieldMappingNode |
...
```
[BrokerNode] → [WatchlistNode] → [HistoricalDataNode] → [ConditionNode] → [NewOrderNode]
```
...
```
[ScheduleNode] → [TradingHoursFilterNode] → [AccountNode] → [ConditionNode] → ...
                                                                     ↑
[BrokerNode] → [RealMarketDataNode] → [ThrottleNode] ───────────────┘
```

### 패턴 3: AI 에이전트 분석

```
[LLMModelNode] ──(ai_model)──→ [AIAgentNode]
                                     ↑
[HistoricalDataNode] ──(tool)───────┘
[MarketDataNode] ──(tool)───────────┘
```
