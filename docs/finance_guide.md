...
* **샘플 코드**: 풍부한 예제 코드는 [GitHub 예제 디렉토리](../src/finance/example/)에서 확인할 수 있습니다.
...
korea.sector()    # 테마 (테마별종목/종목별테마/테마종목별시세 3개 TR) + deprecated 업종 경유 shim
...
# 업종(indtp) 도메인 — /stock 형제 최상위 네임스페이스
indtp = ls.indtp()  # ls.업종() 와 동일. 업종현재가(t1511)/업종기간별추이(t1514)/업종별종목시세(t1516)/업종차트틱(t8408)/업종차트분(t8409)
```
...
* 업종(indtp, `ls.indtp()`/`ls.업종()`): 시세(`/indtp/market-data`) `t1511`(업종현재가), `t1514`(업종기간별추이), `t1516`(업종별종목시세) + 차트(`/indtp/chart`) `t8408`(업종차트틱), `t8409`(업종차트분, ncnt 0=30초/1=1분/n=n분)
* 테마(`ls.국내주식().업종테마()`, `/stock/sector`): `t1531`(테마별종목), `t1532`(종목별테마), `t1537`(테마종목별시세)
...
브로커 상품 구분 없이 공용으로 사용하는 실시간 TR. `Common.real()` 세션을 통해 구독하며 3종 broker(overseas\_stock / overseas\_futureoption / korea\_stock) 중 아무거나 로그인된 상태에서 접근 가능합니다.
...
`IGW00201` 이 응답에 노출되는 경우는 동일 계정으로 **여러 프로세스가 동시 호출** 하여 라이브러리 단일 프로세스 throttle 을 우회한 경우입니다. 이 경우 Redis 등 외부 저장소로 `rate_limit_key` 상태를 공유하는 것이 권장 패턴입니다 (위 [요청 속도 직접 조절](finance_guide.md#요청-속도-직접-조절rate-limiting) 섹션 참조).
...
from programgarden_finance import (
    ...
    t1511, t1514, t1516, t8408, t8409,  # 업종(indtp): ls.indtp()/ls.업종()
