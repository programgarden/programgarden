...
indtp = ls.indtp()  # ls.업종() 와 동일. 업종현재가(t1511)/업종기간별추이(t1514)/업종별종목시세(t1516)/업종차트틱(t8408)
```
...
- 업종(indtp, `ls.indtp()`/`ls.업종()`): 시세(`/indtp/market-data`) `t1511`(업종현재가), `t1514`(업종기간별추이), `t1516`(업종별종목시세) + 차트(`/indtp/chart`) `t8408`(업종차트틱)
...
from programgarden_finance import (
    ...
    t1511, t1514, t1516, t8408,  # 업종(indtp): ls.indtp()/ls.업종()
