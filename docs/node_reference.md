...
>
> **모의투자 설정**: `paper_trading`은 `broker_ls_overseas_futures` credential에서 관리됩니다.
...
로컬 SQLite 데이터베이스에 데이터를 저장/조회합니다. **두 가지 동작 모드**를 지원합니다.

#### 모드 1: execute_query (직접 쿼리)

SQL 쿼리를 직접 작성하여 실행합니다.
...
#### 모드 2: simple (GUI 기반 CRUD)

테이블/액션/컬럼을 선택하여 쿼리를 자동 생성합니다.
...
| 필드 | 타입 | 필수 | 표시 조건 | 설명 |
|------|------|:----:|----------|------|
| `db_name` | string | ✅ | 항상 | DB 파일명 (예: trading.db) |
| `operation` | enum | ✅ | 항상 | `execute_query` 또는 `simple` |
| `query` | string | ✅ | `execute_query` 선택 시 | SQL 쿼리문 |
| `parameters` | object | ❌ | `execute_query` 선택 시 | 쿼리 파라미터 (`:name` 바인딩) |
| `table` | string | ✅ | `simple` 선택 시 | 테이블 이름 |
| `action` | enum | ✅ | `simple` 선택 시 | `select`, `insert`, `update`, `delete`, `upsert` |
| `columns` | string[] | ❌ | `select` 액션 시 | 조회할 컬럼 (미지정 시 전체) |
| `where_clause` | string | ❌ | `select`/`update`/`delete` 시 | WHERE 조건 (예: `symbol = :symbol`) |
| `values` | object | ✅ | `insert`/`update`/`upsert` 시 | 저장할 값 (`:param` 바인딩용 값 포함) |
| `on_conflict` | string[] | ✅ | `upsert` 액션 시 | UPSERT 충돌 컬럼 (예: `["symbol"]`) |

**출력**:
- `rows` - SELECT 결과 배열
- `affected_count` - INSERT/UPDATE/DELETE 영향 받은 행 수

**action 설명**:

| 액션 | 설명 | 필수 필드 |
|------|------|----------|
| `select` | 데이터 조회 | `table`, (선택: `columns`, `where_clause`) |
| `insert` | 데이터 삽입 | `table`, `values` |
| `update` | 데이터 수정 | `table`, `values`, `where_clause` |
| `delete` | 데이터 삭제 | `table`, `where_clause` |
| `upsert` | 삽입 또는 업데이트 | `table`, `values`, `on_conflict` |

**사용 사례**: 트레일링 스탑의 최고점 추적, 매매 이력 저장

> 💡 **데이터 저장 위치**: `programgarden_data/{db_name}` 경로에 자동 생성됩니다.
