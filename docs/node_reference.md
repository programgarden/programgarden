...
### VIXDataNode (VIX 변동성 지수)

Yahoo Finance에서 VIX(CBOE 변동성 지수)를 조회합니다. **credential 불필요**.
...
|------|------|------|
| `vix` | number | 현재 VIX 값 |
| `level` | string | low (< 15) / moderate (< 25) / high (< 35) / extreme (≥ 35) |
| `history` | array | 일별 이력 (`include_history: true` 시) |

> **공통**: 3개 외부 시장 데이터 노드는 모두 retry 기본 활성화, 1분 rate limit, 실시간 노드 직접 연결 차단 (ThrottleNode 경유 필수) 설정을 가집니다.
...
| 카테고리 | 노드 수 | 용도 | 주요 노드 |
|----------|--------|------|----------|
| `infra` | 7 | 시작/연결/흐름/분기 | StartNode, BrokerNode, ThrottleNode, SplitNode, AggregateNode, IfNode |
| `account` | 8 | 계좌 조회 | AccountNode, OpenOrdersNode, RealAccountNode, RealOrderEventNode |
| `market` | 17 | 시세/종목/펀더멘털/환율/심리/변동성/제외종목 | MarketDataNode, FundamentalNode, HistoricalDataNode, RealMarketDataNode, WatchlistNode, ScreenerNode, SymbolFilterNode, ExclusionListNode, CurrencyRateNode, FearGreedIndexNode, VIXDataNode |
...
- [종목조건 플러그인](strategies/stock_condition.md) - RSI, MACD 등 55개 전략 플러그인
