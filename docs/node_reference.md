...
> **노드 수**: 11개 카테고리, 51개 노드
>
> **해외주식 / 해외선물**: 대부분의 노드가 상품별로 분리되어 있습니다. 예를 들어 `OverseasStockBrokerNode`(해외주식)와 `OverseasFuturesBrokerNode`(해외선물)는 같은 기능이지만 상품이 다릅니다.
...
2. [account - 계좌 조회](#2-account---계좌-조회)
3. [market - 시세/종목](#3-market---시세종목)
4. [condition - 조건 평가](#4-condition---조건-평가)
5. [order - 주문 실행](#5-order---주문-실행)
6. [risk - 리스크 관리](#6-risk---리스크-관리)
7. [schedule - 스케줄/시간](#7-schedule---스케줄시간)
8. [data - 데이터 조회/저장](#8-data---데이터-조회저장)
9. [display - 시각화](#9-display---시각화)
10. [analysis - 백테스트/분석](#10-analysis---백테스트분석)
11. [ai - AI 에이전트](#11-ai---ai-에이전트)
...
| 상품 | 노드 타입 | 모의투자 |
|------|----------|---------|
| 해외주식 | `OverseasStockBrokerNode` | 미지원 (실전만 가능) |
| 해외선물 | `OverseasFuturesBrokerNode` | 지원 |
...
> **주의 - 해외주식 모의투자**: LS증권은 해외주식 모의투자를 지원하지 않습니다. 해외주식은 항상 실전 계좌로만 거래됩니다. 모의투자를 먼저 해보고 싶다면 해외선물(`OverseasFuturesBrokerNode`)로 시작하세요.
...
```
WatchlistNode ──(AAPL, NVDA, TSM)──▶ SplitNode ──(1개씩)──▶ HistoricalDataNode
                                                                  ↓ 1회차: AAPL
                                                                  ↓ 2회차: NVDA
                                                                  ↓ 3회차: TSM
```
...
```
WatchlistNode → SplitNode → HistoricalDataNode → ConditionNode → AggregateNode
  (3종목)        (분리)       (각각 조회)          (각각 평가)      (결과 취합)
```
...
```
RealOrderEventNode ──(filled)──▶ TableDisplayNode
                   ──(rejected)──▶ TableDisplayNode (에러 로그)
```
...
```
MarketUniverseNode ──┐
                     ├──▶ SymbolFilterNode (difference) ──▶ ...매수 로직
AccountNode ─────────┘     "나스닥100 중 보유하지 않은 종목만"
```
...
```
BrokerNode → WatchlistNode → SplitNode → HistoricalDataNode → ConditionNode (RSI)
                                                                     ↓
                                                              LogicNode (AND)
                                                                     ↓
                                                              NewOrderNode
...
ConditionNode ──(종목)───┘     "잔고의 10%로 계산"           "계산된 수량으로 주문"
```
...
| 카테고리 | 노드 수 | 용도 | 주요 노드 |
|----------|--------|------|----------|
| `infra` | 6 | 시작/연결/흐름 | StartNode, BrokerNode, ThrottleNode, SplitNode, AggregateNode |
| `account` | 8 | 계좌 조회 | AccountNode, OpenOrdersNode, RealAccountNode, RealOrderEventNode |
| `market` | 12 | 시세/종목 | MarketDataNode, HistoricalDataNode, RealMarketDataNode, WatchlistNode, ScreenerNode, SymbolFilterNode |
| `condition` | 2 | 조건 평가 | ConditionNode, LogicNode |
| `order` | 7 | 주문 실행 | NewOrderNode, ModifyOrderNode, CancelOrderNode, PositionSizingNode |
| `risk` | 1 | 리스크 관리 | PortfolioNode |
| `schedule` | 2 | 스케줄/시간 | ScheduleNode, TradingHoursFilterNode |
| `data` | 3 | 데이터 조회/저장 | SQLiteNode, HTTPRequestNode, FieldMappingNode |
...
- [종목조건 플러그인](strategies/stock_condition.md) - RSI, MACD 등 17개 전략 플러그인
