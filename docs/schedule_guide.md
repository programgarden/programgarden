...
> **📌 표기 원칙**: 본 가이드 및 다른 모든 ProgramGarden 문서의 cron 예제는 **6-field 포맷(`초 분 시 일 월 요일`)** 으로 통일합니다. 다른 파이썬 croniter 기반 라이브러리에서 흔히 쓰는 5-field(`분 시 일 월 요일`)도 지원되지만, 문서 예제는 항상 초 필드를 명시하여 혼동을 방지합니다.
...
- **6개 필드 (권장, 문서 표준)**: `초 분 시 일 월 요일`
  - 예: `0 30 9 * * *` → 매일 09:30:00
- 5개 필드: `분 시 일 월 요일` (초가 없음, 항상 0초로 동작)
  - 예: `30 9 * * *`  → 매일 09:30:00 (초는 항상 0)
- 7개 필드: `초 분 시 일 월 요일 연도`
  - 예: `0 0 0 1 1 * 2026` → 2026년 1월 1일 00:00:00

> **⚠️ 혼동 방지**: 다른 가이드·튜토리얼에서 5-field와 6-field가 혼용되어 "초 필드를 넣어야 하는지" 헷갈리는 경우가 있습니다. **ProgramGarden 문서 전반은 6-field를 기본으로 사용**하며, 각 예제 상단 또는 주석에 "6-field (`초 분 시 일 월 요일`)"를 명시합니다.
